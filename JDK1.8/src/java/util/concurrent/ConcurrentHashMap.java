...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    static final <K, V> Node<K, V> tabAt(Node<K, V>[] tab, int i) {
        //此函数返回table数组中下标为i的结点，可以看到是通过Unsafe对象通过反射获取的，
     ...
     */
    //initTable、tabAt、casTabAt、helpTransfer、putTreeVal、treeifyBin、addCount
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        for (int i = 0, bound = 0; ; ) {
            ...
            if (i < 0 || i >= n || i + n >= nextn) {
                ...
                /**
                 * ，transfer的for循环是倒叙的，说明对table的遍历是从table.length-1开始到0的。我觉得这段代码写得太牛逼了，特别是
                 */
                ...
                synchronized (f) {
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            setTabAt(tab, i, fwd);
                            advance = true;
                        } else if (f instanceof TreeBin) {//下面红黑树基本和链表差不多
                            TreeBin<K, V> t = (TreeBin<K, V>) f;
        ...
        for (; ; ) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                if ((a = as[(n - 1) & h]) == null) {
                    ...
                    collide = false;            // At max size or stale表明as已经过时，说明cells已经初始化完成，看下面，重置collide为false表明已经存在竞争
                else if (!collide)
                    collide = true;
                else if (cellsBusy == 0 &&
                        U.compareAndSwapInt(this, CELLSBUSY, 0, 1)) {
                    try {
                        if (counterCells == as) {// Expand table unless stale下面的代码主要是给counterCells扩容，尽可能避免冲突
     ...
     *
     * 此函数用于将桶中的数据结构转化为红黑树，其中，值得注意的是，当table的长度未达到阈值时，会进行一次扩容操作，该操作会使得
     * 触发treeifyBin操作的某个桶中的所有元素进行一次重新分配，这样可以避免某个桶中的结点数量太大。
     */
